...
void DJISDKNode::broadcast_callback()
{
    ...
    if ( (msg_flags & HAS_POS) && (msg_flags & HAS_Q) && (msg_flags & HAS_W) && (msg_flags & HAS_V) ) {
        ...
        transform.setOrigin(tf::Vector3(local_position.x,
                                        local_position.y,
                                        local_position.z)); // Note z axis is inverted
        transform.setRotation(tf::Quaternion(attitude_quaternion.q1,
                                             attitude_quaternion.q2,
                                             attitude_quaternion.q3,
                                             attitude_quaternion.q0)); // x y z w


        tf_broadcaster.sendTransform(tf::StampedTransform(transform,
                                                            current_time, "/world", "/base_link"));

        // THIS IS WHERE I FUCK EVERytHING UP
    }
...
******************************************************************/
        ...
        if (msg_flags & HAS_GIMBAL) {
            tf::Transform transform;
            tf::Quaternion q;
            transform.setOrigin(tf::Vector3(0.13, 0.0, 0.075));
            // This setEuler has issues
            //q.setEuler(DEG2RAD(bc_data.gimbal.yaw), DEG2RAD(bc_data.gimbal.pitch), DEG2RAD(bc_data.gimbal.roll));
            q.setRPY(DEG2RAD(bc_data.gimbal.roll), DEG2RAD(bc_data.gimbal.pitch), DEG2RAD(bc_data.gimbal.yaw));
            transform.setRotation(q);

            tf_broadcaster.sendTransform(
                        tf::StampedTransform(transform,
                            current_time, "/m100", "/gimbal"));
        }
    ...
    transform.setRotation(tf::Quaternion(0.0, 0.0, 0.0, 0.70710678118));

    tf_broadcaster.sendTransform(
                tf::StampedTransform(transform,
                    current_time, "/base_link", "/m100"));

    transform.setOrigin(tf::Vector3(0.0, 0.0, 0.0));
    q.setRPY(C_PI, 0.0, 0.0);
    transform.setRotation(q);

    tf_broadcaster.sendTransform(
                tf::StampedTransform(transform,
                    current_time, "/map", "/world"));
